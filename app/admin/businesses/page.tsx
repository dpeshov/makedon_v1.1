...
export default async function AdminBusinessesPage({
  ...
  let query = supabase
    .from("businesses")
    .select("id, company_name, country, city, email, created_at, approval_status, user_id")
    ...
    <div className="flex flex-col gap-6">
      <Surface className="p-7 sm:p-10">
        <div className="flex flex-col gap-4 sm:flex-row sm:items-end sm:justify-between">
          <div>
            ...
            <p className="mt-2 text-sm text-slate-600">Review, approve or reject business registrations.</p>
      ...
      {listRes.error ? (
        <Surface className="p-5 sm:p-6">
          <p className="text-sm text-rose-800">Could not load businesses: {listRes.error.message}</p>
        </Surface>
      ) : rows.length === 0 ? (
        <Surface className="p-7 sm:p-10">
              ...
              <div key={r.id} className="flex flex-col gap-3 p-4 sm:flex-row sm:items-center sm:justify-between">
                <div className="min-w-0">
                  <div className="flex flex-wrap items-center gap-2">
                    <Link className="truncate font-semibold text-slate-900 underline underline-offset-4" href={`/admin/businesses/${r.id}`}>
                      {r.company_name}
                    </Link>
                ...
                <div className="flex flex-wrap gap-2 sm:justify-end">
                  <ButtonLink href={`/admin/businesses/${r.id}`} variant="secondary" size="sm">
                    View
                  </ButtonLink>
