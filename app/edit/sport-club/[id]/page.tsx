...
export default function EditSportClubPage() {
  ...
  useEffect(() => {
    ...
    async function run() {
      ...
      const res = await supabase
        .from("sport_clubs")
        .select(
          "id, club_name, contact_name, country, city, sport, description, phone, address, website, email, training_schedule, age_groups, league, facebook, instagram, approval_status"
        )
        .eq("id", id)
        .maybeSingle();
      if (cancelled) return;
    ...
    try {
      ...
      const res = await supabase
        ...
        .update({
          ...
          email: row.email,
          training_schedule: row.training_schedule || null,
  ...
  if (!row) {
    return (
      <Surface className="p-7 sm:p-10">
        <h1 className="text-2xl font-semibold text-slate-900 [font-family:var(--font-heading)]">Edit sport club</h1>
    ...
    <Surface className="p-5 sm:p-6">
      ...
      <form onSubmit={onSave} className="mt-6">
        <div className="grid gap-4 sm:grid-cols-2">
          ...
          <label className="text-sm font-medium text-slate-700">
            Sport
            <input className={inputClassName()} value={row.sport} onChange={(e) => setRow({ ...row, sport: e.target.value })} required />
          </label>
          <label className="text-sm font-medium text-slate-700">
            Website (optional)
        ...
        <div className="mt-4 grid gap-4 sm:grid-cols-2">
          <label className="text-sm font-medium text-slate-700">
            ...
            <input className={inputClassName()} value={row.training_schedule ?? ""} onChange={(e) => setRow({ ...row, training_schedule: e.target.value })} />
          </label>
          <label className="text-sm font-medium text-slate-700">
            Age groups (optional)
            <input className={inputClassName()} value={row.age_groups ?? ""} onChange={(e) => setRow({ ...row, age_groups: e.target.value })} />
          </label>
          <label className="text-sm font-medium text-slate-700">
            League (optional)
            <input className={inputClassName()} value={row.league ?? ""} onChange={(e) => setRow({ ...row, league: e.target.value })} />
          </label>
          <label className="text-sm font-medium text-slate-700">
            Address (optional)
