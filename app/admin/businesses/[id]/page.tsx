...
async function setApproval(id: string, next: "approved" | "rejected") {
  ...
  const { error } = await (res.supabase as any).from("businesses").update(patch).eq("id", id);
  ...
  if (!row) {
    return (
      <Surface className="p-7 sm:p-10">
        ...
        <div className="mt-6">
          <ButtonLink href="/admin/businesses" variant="secondary">
            Back
          </ButtonLink>
        </div>
      </Surface>
  ...
  return (
    <div className="flex flex-col gap-6">
        ...
        <div className="flex flex-col gap-4 sm:flex-row sm:items-end sm:justify-between">
          <div className="min-w-0">
            ...
            <p className="mt-2 text-sm text-slate-600">
              Back to{" "}
              <Link className="font-semibold underline underline-offset-4" href="/admin/businesses">
                businesses
              </Link>
              .
            </p>
      ...
      <div className="grid gap-3 sm:grid-cols-2">
        ...
        <Field label="Owner name" value={row.owner_name} />
        <Field label="Industry" value={row.industry} />
        <Field label="Sub industry" value={row.sub_industry} />
        ...
        <Field label="Other locations" value={row.other_locations} />
        <Field label="Locations description" value={row.locations_description} />
        <Field label="Offerings" value={row.offerings} />
        <Field label="Offerings description" value={row.offerings_description} />
      </div>
